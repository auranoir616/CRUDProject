  ...
  <main>
    ...
    <article>
      ...
      <ul class="nav justify-content-center">
         <li class="nav-item">
          <a class="nav-link" aria-current="page" href="#" onclick="resultPost(); event.preventDefault();">Posts</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="#" onclick="resultUser(); event.preventDefault();">Users</a>
        </li>
      </ul>
      <hr>
      
      @if ($resultsPost->isEmpty() && $resultsUser->isEmpty())
      <h2>No Result</h2>
      @else
      <div id="container-resultPost">
          @foreach ($resultsPost as $data)
          <?php 
            $usernameLink = DB::table('users')->where('id', $data->user_id)->first();
          ?>
          <div class="row g-0 position-relative" id="container-post">
            <div>
              <a @if($data->user_id == auth()->id()) href="/myprofile" @endif href="/profile/{{$data->user_id}}-{{$usernameLink->username}}" class="link-light link-offset-2 link-offset-3-hover link-underline link-underline-opacity-0 link-underline-opacity-75-hover">
              <b>{{$usernameLink->username}}</b></a>
               <br>
      ...
      @endforeach
          ...
          <div  id="container-resultUser">
            @foreach($resultsUser as $result)
            <div class="w-100 p-3">          
              <div class="row g-0 position-relative">
                <div class="col-md-6 mb-md-0 p-md-4">
                @if(isset($result->Images_profile))
                <img src="./data_file/{{$result->Images_profile}}" id="image-result" alt="{{$result->Images_profile}}">
                @endif
              </div>
              <div class="col-md-6 p-4 ps-md-0">
                <div class="SearchResult-body">
                  <a href="/profile/{{$result->id}}-{{$result->username}}" class="link-light link-offset-2 link-underline-opacity-25 link-underline-opacity-100-hover">
                    <h4>{{$result->name}}</h4></a>
                  <p class="card-text">{{$result->bio}}</p>
                </div>
                <br>
                <a href="/messages/{{$result->id}}" class="btn btn-outline-secondary btn-sm">message</a>
                  <button type="submit" @if(auth()->user()->followedUser->contains($result->id)) class="btn btn-secondary btnFollow btn-sm" @endif class="btn btn-light btnFollow btn-sm" following_user_id="{{$result->id}}">
                    {{ auth()->user()->followedUser->contains($result->id) ? 'Unfollow' : 'Follow' }}
                  </button>
              </div>
            </div>
...
//     }, 1500);
let user = document.getElementById('container-resultUser')
let post = document.getElementById('container-resultPost')
function resultPost(){
  user.hidden = true
  post.hidden = false
}
