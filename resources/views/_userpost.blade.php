        ...
        {{--! form edit & delete --}}
          <form action="/deletepost/{{$data->id}}" method="POST">
            ...
            @method('DELETE')
              ...
              <li><a class="dropdown-item" href="#"  data-bs-toggle="modal" data-bs-target="#staticBackdrop{{$data->id}}">Edit</a></li>
...
<hr>
    ...
    @endif
    <br>
    <div id="likesCountDisplay{{$data->id}}">
      <div>
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            ...
            <div class="modal-body">
              <form action="/editpost/{{$data->id}}" method="POST" enctype="multipart/form-data"> 
                @csrf
                @method('PUT')
          <div class="mb-3">
            <label for="exampleFormControlInput1" class="form-label">images</label>
            <input type="file" class="form-control" id="exampleFormControlInput1" name="images"  value="{{$data->images}}">
          </div>
          <div class="mb-3">
            <label for="exampleFormControlTextarea1" class="form-label">description</label>
            <textarea class="form-control" id="exampleFormControlTextarea1" rows="3" name="body">{{$data->body}}</textarea>
          </div>
        </div>
...
function likesCount(postId){
  ...
  .then(data => {
    console.log(data)
    let display = document.getElementById(`likesCountDisplay${postId}`)
    if(data.CountLikesPost !== 0){
